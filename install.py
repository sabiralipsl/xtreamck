...
rDownloadURL = {
    "main": "https://www.dropbox.com/scl/fi/6rcbztyzkytcp20017wsa/main_xui_masoudgb.zip?rlkey=s8v1vzt2u9r1dhop4ovfz44md&st=rauptto6&dl=1",
    "sub": "https://www.dropbox.com/scl/fi/s235q2v2o0afuit3tmnz8/sub_xui_masoudgb.zip?rlkey=72xojuilrtp13vbxqign71ni5&st=gjeibnbe&dl=1",
    "update": "https://bitbucket.org/masoudgb/xtream-ui/raw/master/update.zip",
    "release": "https://www.dropbox.com/scl/fi/ck0aaq1bf071p4ru7goxc/release_22f.zip?rlkey=etumg855lpybgk3e4dvvqb0b2&st=ujtyybux&dl=1"
}

rPackages = ["libcurl4", "libxslt1-dev", "libgeoip-dev", "libonig-dev", "e2fsprogs", "wget", "mcrypt", "nscd", "htop", "zip", "unzip", "mc", "mariadb-server", "libpng16-16", "python3-paramiko", "python-is-python3", "curl"]
...
def stop_services():
    subprocess.run("killall -9 nginx nginx_rtmp php-fpm > /dev/null 2>&1", shell=True)
    subprocess.run("pkill -9 -f xtreamcodes > /dev/null 2>&1", shell=True)
    import time
    time.sleep(1)
...
def prepare(rType="MAIN"):
    ...
    if os.path.exists("/home/xtreamcodes"):
        stop_services()
        subprocess.run("chattr -R -i /home/xtreamcodes/ > /dev/null 2>&1", shell=True)
    if os.path.isfile('/home/xtreamcodes/iptv_xtream_codes/config'): shutil.copyfile('/home/xtreamcodes/iptv_xtream_codes/config', '/tmp/config.xtmp')
    if os.path.isfile('/home/xtreamcodes/iptv_xtream_codes/GeoLite2.mmdb'): os.system('chattr -i /home/xtreamcodes/iptv_xtream_codes/GeoLite2.mmdb > /dev/null 2>&1')
    ...
    if not is_installed("curl"):
        ...
        subprocess.run("apt-get install curl -y > /dev/null 2>&1", shell=True)
    if rType == "MAIN":
        ...
        subprocess.run("curl -fsSL https://mariadb.org/mariadb_release_signing_key.asc | gpg --dearmor --yes -o /usr/share/keyrings/mariadb-archive-keyring.gpg > /dev/null 2>&1", shell=True)
    ...
    if not is_installed("libssl1.1"):
        ...
        subprocess.run("curl -L -o /tmp/libssl1.1.deb http://archive.ubuntu.com/ubuntu/pool/main/o/openssl/libssl1.1_1.1.0g-2ubuntu4_amd64.deb > /dev/null 2>&1 && sudo dpkg -i /tmp/libssl1.1.deb > /dev/null 2>&1 && rm -rf /tmp/libssl1.1.deb > /dev/null 2>&1", shell=True)
    if not is_installed("libzip5"):
        printc("Installing libzip5")
        subprocess.run("curl -L -o /tmp/libzip5.deb http://archive.ubuntu.com/ubuntu/pool/universe/libz/libzip/libzip5_1.5.1-0ubuntu1_amd64.deb > /dev/null 2>&1 && sudo dpkg -i /tmp/libzip5.deb > /dev/null 2>&1 && rm -rf /tmp/libzip5.deb > /dev/null 2>&1", shell=True)
    ...
    if not python_installed or not pip_installed or not paramiko_installed:
        ...
        if not python_installed: subprocess.run("cd /usr/src && sudo curl -L -o Python-2.7.18.tgz https://www.python.org/ftp/python/2.7.18/Python-2.7.18.tgz > /dev/null 2>&1 && sudo tar xzf Python-2.7.18.tgz > /dev/null 2>&1 && cd Python-2.7.18 && sudo ./configure --enable-optimizations > /dev/null 2>&1 && sudo make altinstall > /dev/null 2>&1", shell=True)
        if not pip_installed: subprocess.run("curl -L -o get-pip.py https://bootstrap.pypa.io/pip/2.7/get-pip.py > /dev/null 2>&1 && sudo python2.7 get-pip.py > /dev/null 2>&1", shell=True)
        if not paramiko_installed: subprocess.run("pip2.7 install paramiko > /dev/null 2>&1", shell=True)
    subprocess.run("apt-get install -f -y > /dev/null 2>&1", shell=True)
    try: subprocess.run("getent passwd xtreamcodes > /dev/null 2>&1", shell=True, check=True)
    except subprocess.CalledProcessError:
...
def install(rType="MAIN"):
    ...
    try: result = subprocess.run(['curl', '-L', '-o', zip_file_path, '-s', '--max-time', '300', rURL], check=True, capture_output=True)
    ...
    if os.path.exists(zip_file_path) and os.path.getsize(zip_file_path) > 0:
        ...
        stop_services()
        if os.path.exists("/home/xtreamcodes/iptv_xtream_codes/GeoLite2.mmdb"): subprocess.run("chattr -i /home/xtreamcodes/iptv_xtream_codes/GeoLite2.mmdb > /dev/null 2>&1", shell=True)
        subprocess.run("chattr -R -i /home/xtreamcodes/ > /dev/null 2>&1", shell=True)
        try:
            with zipfile.ZipFile(zip_file_path, 'r') as zip_ref: zip_ref.extractall("/home/xtreamcodes/")
        except zipfile.BadZipFile:
            printc("Error: Invalid zip file!", col.BRIGHT_RED)
            return False
...
def update(rType="MAIN", customURL=None):
    global rDownloadURL
    if customURL: rlink = customURL
    elif rType == "UPDATE": rlink = rDownloadURL["update"]
    else: rlink = rDownloadURL["release"]
    printc("Downloading Software Update")
    result = subprocess.run(['curl', '-L', '-o', '/tmp/update.zip', '-s', '--max-time', '300', rlink], capture_output=True)
    if os.path.exists("/tmp/update.zip") and os.path.getsize("/tmp/update.zip") > 0:
        ...
        printc("Failed to download update file!", col.BRIGHT_RED)
        return False
...
def encrypt(rHost="127.0.0.1", rUsername="user_iptvpro", rPassword="", rDatabase="xtream_iptvpro", rServerID=1, rPort=7999, useExisting=False):
    if useExisting and os.path.isfile('/home/xtreamcodes/iptv_xtream_codes/config'):
        rDecrypt = decrypt()
        if rDecrypt:
            rHost = rDecrypt["host"]
...
def fixPermissions():
    printc("Fixing Permissions")
    os.system("rm -f /home/xtreamcodes/iptv_xtream_codes/php/*.pid > /dev/null 2>&1")
    os.system("chmod 755 /home/xtreamcodes/iptv_xtream_codes/php > /dev/null 2>&1")
    os.system("chown -R xtreamcodes:xtreamcodes /home/xtreamcodes/iptv_xtream_codes/php > /dev/null 2>&1")
    os.system("sed -i 's|chmod -R 0777 /home/xtreamcodes|chmod -R 0755 /home/xtreamcodes|g' /home/xtreamcodes/iptv_xtream_codes/permissions.sh > /dev/null 2>&1")
    os.system("sed -i 's|sudo chmod -R 777 /home/xtreamcodes|sudo chmod -R 755 /home/xtreamcodes|g' /home/xtreamcodes/iptv_xtream_codes/permissions.sh > /dev/null 2>&1")
    if os.path.exists("/home/xtreamcodes/iptv_xtream_codes/start_services.sh"):
        os.system("sed -i 's|chmod -R 777 /home/xtreamcodes|chmod -R 755 /home/xtreamcodes|g' /home/xtreamcodes/iptv_xtream_codes/start_services.sh > /dev/null 2>&1")
        os.system("sed -i 's|chmod 777 /home/xtreamcodes|chmod 755 /home/xtreamcodes|g' /home/xtreamcodes/iptv_xtream_codes/start_services.sh > /dev/null 2>&1")
...
def configure():
    ...
    if not "/home/xtreamcodes/iptv_xtream_codes/" in open("/etc/fstab").read():
        ...
        rFile.close()
    if not "xtreamcodes" in open("/etc/sudoers").read(): os.system('echo "xtreamcodes ALL = (root) NOPASSWD: /sbin/iptables, /usr/bin/chattr" >> /etc/sudoers')
    ...
    os.system("chmod -R 0755 /home/xtreamcodes > /dev/null")
    os.system("chattr -ai /home/xtreamcodes/iptv_xtream_codes/GeoLite2.mmdb > /dev/null")
    os.system("sudo chmod 0755 /home/xtreamcodes/iptv_xtream_codes/GeoLite2.mmdb > /dev/null")
...
def fixPHPExtensions():
    PHP_EXT_DIR = "/home/xtreamcodes/iptv_xtream_codes/php/lib/php/extensions/no-debug-non-zts-20180731"
    PHP_INI = "/home/xtreamcodes/iptv_xtream_codes/php/lib/php.ini"
    PHP_ETC = "/home/xtreamcodes/iptv_xtream_codes/php/etc"
    ioncube_73 = f"{PHP_EXT_DIR}/ioncube_loader_lin_7.3.so"
    if not os.path.exists(ioncube_73) or os.path.getsize(ioncube_73) < 100000:
        os.system("cd /tmp && rm -rf ioncube* > /dev/null 2>&1")
        subprocess.run(['curl', '-L', '-s', '-o', '/tmp/ioncube_loaders.tar.gz', 'https://downloads.ioncube.com/loader_downloads/ioncube_loaders_lin_x86-64.tar.gz'], capture_output=True)
        if os.path.exists("/tmp/ioncube_loaders.tar.gz") and os.path.getsize("/tmp/ioncube_loaders.tar.gz") > 0:
            os.system("cd /tmp && tar -xzf ioncube_loaders.tar.gz > /dev/null 2>&1")
            if os.path.exists("/tmp/ioncube/ioncube_loader_lin_7.3.so"):
                shutil.copy("/tmp/ioncube/ioncube_loader_lin_7.3.so", ioncube_73)
                os.system(f"chmod 755 {ioncube_73}")
            os.system("rm -rf /tmp/ioncube* > /dev/null 2>&1")
    geoip_file = f"{PHP_EXT_DIR}/geoip.so"
    if os.path.exists(geoip_file):
        try: os.remove(geoip_file)
        except: pass
    if os.path.exists(PHP_INI):
        with open(PHP_INI, 'r') as f: php_ini_content = f.read()
        shutil.copy(PHP_INI, f"{PHP_INI}.backup")
        php_ini_content = php_ini_content.replace('ioncube_loader_lin_7.2.so', 'ioncube_loader_lin_7.3.so')
        php_ini_content = php_ini_content.replace('extension=geoip.so', ';extension=geoip.so')
        php_ini_content = php_ini_content.replace('extension="geoip.so"', ';extension="geoip.so"')
        php_ini_content = php_ini_content.replace('extension=mcrypt.so', ';extension=mcrypt.so')
        php_ini_content = php_ini_content.replace('extension="mcrypt.so"', ';extension="mcrypt.so"')
        if 'zend_extension' not in php_ini_content or 'ioncube' not in php_ini_content:
            php_ini_content = f'zend_extension = "{PHP_EXT_DIR}/ioncube_loader_lin_7.3.so"\n' + php_ini_content
        with open(PHP_INI, 'w') as f: f.write(php_ini_content)
    if os.path.exists(PHP_ETC):
        for filename in os.listdir(PHP_ETC):
            filepath = os.path.join(PHP_ETC, filename)
            if os.path.isfile(filepath) and filename.endswith('.conf'):
                try:
                    with open(filepath, 'r') as f: content = f.read()
                    if 'ioncube_loader_lin_7.2.so' in content:
                        content = content.replace('ioncube_loader_lin_7.2.so', 'ioncube_loader_lin_7.3.so')
                        with open(filepath, 'w') as f: f.write(content)
                except: pass
    os.system(f"chmod 755 {PHP_EXT_DIR}/*.so > /dev/null 2>&1")
...
def start(first=True):
    ...
    fixPermissions()
    os.system("nohup /home/xtreamcodes/iptv_xtream_codes/start_services.sh > /dev/null 2>&1 &")
...
"""
        ...
        if http_start_index != -1:
            ...
            with open(rPath, "w") as rFile: rFile.write(rData)

def doUpdate():
    global rDownloadURL
    print("  Select update source:")
    print("  [1] Default URL (bitbucket.org)")
    print("  [2] Custom URL")
    print(" ")
    ...
    customURL = None
    if choice == "2":
        customURL = input("  Enter custom update URL: ").strip()
        print(" ")
        if not customURL:
            printc("Invalid URL!", col.BRIGHT_RED)
            return False
    if not update("UPDATE", customURL): return False
    fixPHPExtensions()
    fixPermissions()
    return True
...
if __name__ == "__main__":
    ...
    if not rVersion in rVersions:
        ...
        sys.exit(1)
    printc("X-UI installer in Ubuntu %s - ck41 by sabir ali" % rVersion, col.GREEN, 2)
    print(" ")
    ...
    if rType.upper() in ["MAIN", "LB"]:
        ...
        if len(rHost) > 0 and len(rPassword) > 0 and rServerID > -1:
            ...
            if input("  ").upper() == "Y":
                ...
                if rType.upper() == "MAIN":
                    ...
                    printc("Admin UI: http://%s:25500" % getIP(), col.BRIGHT_YELLOW)
                    printc("Admin UI default login is admin/admin", col.BRIGHT_YELLOW)
                    printc("Credentials saved to /root/credentials.txt", col.BRIGHT_YELLOW)
                    ...
                    rFile.close()
                    print(" ")
                    printc("Do you want to update now? Y/N", col.BRIGHT_YELLOW)
                    if input("  ").upper() == "Y":
                        print(" ")
                        if doUpdate():
                            start(False)
                            printc("Update completed!", col.GREEN, 2)
        ...
        if os.path.exists("/home/xtreamcodes/iptv_xtream_codes/wwwdir/api.php"):
            printc("Update Xtream-UI? Y/N", col.BRIGHT_YELLOW)
            if input("  ").upper() == "Y":
                print(" ")
                if doUpdate():
                    start(False)
